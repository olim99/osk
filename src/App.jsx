    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <main className="p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <button className="border px-4 py-1 rounded">Move All To Bag</button>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <div key={product.id} className="border rounded shadow p-4 relative">
              {product.aksiya && (
              ...
              <div className="text-yellow-500 text-sm">
                {"★".repeat(Math.floor(product.reting)) +
                  (product.reting % 1 >= 0.5 ? "★" : "") +
                  "☆".repeat(5 - Math.ceil(product.reting))}
              </div>
              <button className="mt-4 bg-black text-white w-full py-1 rounded text-sm">
                Add To Cart
              </button>
            </div>
          ))}
